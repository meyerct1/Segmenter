...
numChannels = 4; %Including nuclei and BF channels
imExt = '.jpg';
...
for i = 1:numCh
    ...
    if (cidrecorrect)
       Cyto.(str).CIDREmodel.v = csvread('CIDRE Model');
    end
end
...
%     tim.min = str2double(NUC.filnms(i).name(11:12));
    ...
    NUCim = im2uint16(NUCim);

    %Store for later use in the code
    tempNuc = NUCim;
    ...
    % Segment properties
    p	= regionprops(CYP_label,'PixelIdxList','Perimeter');
    ...
    for q = 1:numCh
       ...
       %Correct the intensity in this channel by the correction amount on a per pixel basis 
       if strcmp(chnm,corCH)
           CYPim = CYPim - (corVal/100*tempNuc);
       end
