function [] = MultiChannelSegmentation_v8(imExt)
...
%{
    im_name = ['20151029143827-11716-R02-C02' imExt];
    ...
    singleSegFig(im,numCh,imExt,cidrecorrect,numLevels,surface_segment,perimeter_size_disk,tophat_rad,...
              nuc_segment,nuc_dil_disk,corCH,corVal,noise_disk,NUC,Cyto,BitDepth,cl_border)
%}  
...
%         end
    ...
    if nuc_segment==1
        ...
        for q = 1:numCh
            ...
            try num = multithresh(Im_array(:,:,q+1),numLevels); 
                ...
                while cnt<numLevels
                  try num = multithresh(Im_array(:,:,q+1),numLevels-cnt); break; end
                  cnt = cnt+1;
                end
            end
            %Run Otsu's method
            %Quantize the image based on multithreshold.  Set every level above 1 to signal and 1 to background (0)
            tempIm= imquantize(Im_array(:,:,q+1), num); 
            tempIm(tempIm == 1) = 0; %Background
            %every other level is a cell
            tempIm(tempIm>1)=1;
            % Remove Noise
    ...
    if cl_border == 1
        ...
        temp = [CytoLabel(1,:)   CytoLabel(:,size(CytoLabel,2))'   CytoLabel(size(CytoLabel,1),:)  CytoLabel(:,1)'];
        temp = unique(temp(temp~=0));
