% Multi Channel Cell Segmentation based only on nucleus
%%Christian Meyer 10/16/15
...
%First nuclei are segmented. Then doubled using an Imdilate to roughly define
%cell boundaries. The intensity, area, and
...
cidrecorrect = 0; %don't correct with cidre model
numLevels = 5;  %Number of levels for segmentation using otsu's method for cytoplasm segmentation
nuc_dil_disk = 5
mean_background_level = 250;
...
%the most amount of cell area is picked as defining the cytoplasm in
%addition to adding in the segmented nuclei dialated by the nuc_dil_factor
%disk structure element.
...
%         end
        ...
        if max(max(Im_array(:,:,q+1))) > mean_background_level; %Approximately 20% of max 
    ...
    Label_array = Label_array + im2double(imdilate(Nuc_label,strel('disk',nuc_dil_disk))); %Add in the nucleus for each cytoplasm
...
%   for k = 1:length(p)
%       loc = p(k).Centroid;
%       t = text(loc(1),loc(2),num2str(Int(k)))
%       t.Color = [1 1 1]
%   end
%   str = sprintf('EpCAM-AF488 max conc. image=%i',i)
%   title(str)
%   set(gca,'fontsize',14,'color',[1 1 1])
%   tightfig()
%   figure
%   imshow(label2rgb(CytoLabel),[])
%   title('Cytoplasm segmentation i = 32')
%   
%   figure
%   subplot(1,2,1)
%   [x,y] = ksdensity(Int)
%   plot(y,x,'linewidth',4)
%   str = sprintf('EpCAM-AF488 Intensity im%i',i)
%   title(str)
%   xlabel('Intensity')
%   subplot(1,2,2)
%   [x,y] = ksdensity(Int./Area)
%   plot(y,x,'linewidth',4,'color','r')
%   str = sprintf('EpCAM-AF488 Intensity per area im%i',i)
%   title(str)
%   xlabel('I/A')
%   
