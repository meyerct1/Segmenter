...
%     tim.min = str2double(NUC.filnms(i).name(11:12));
    ...
    %Now segement each Channel based on the nuclear location
    for q = 1:numCh
        chnm = ['CH_' num2str(q)];
...
%         CYPim(CYPim ==3) = 1;
        ...
        CO.(chnm).numCyto = length(unique(CYP_label))-1;
        CO.(chnm).Intensity = Int;
        CO.(chnm).Area = Area;
        CO.(chnm).label = CYP_label;
        CO.(chnm).numCytowoutNuc = numCytowoutNuc;
    end
