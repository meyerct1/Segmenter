clear all
ImageDirectory='/Users/sg_hummel/Dropbox/FocalAdhesions/Cells to analyze/Individual timepoint stacks/Cell 02 110112 GFP-Vinc/';
Output='/Users/sg_hummel/Documents/Test/';
ImageRoot='Cell 02_t';
StartFrame=1;
FrameCount=21;
TimeFrame=1;
...
ImageExtension='.TIF';
MaxMissingFrames=3;
OutputDirectory=[Output 'OutputTest2/'];
% Adjustable Parameters for processing the images
BrightnessThresholdPct= 1.35; % 1.35; %1.25; %1.45 %1.6
...
%%%%%%%%
if isempty(Potential_FAs) % end at 299
...
% Distnce Search through depth
     ...
     if R == 1 % R == 1
         ...
         Search_Data = New_sub(New_sub(:,3) == ARR(R-1),:); 
         if isempty(Search_Data)
             Search_Data = New_sub(New_sub(:,3) == AR-2,:);
         end
         
         [Index, Distance] = knnsearch(Search_Data(:,5:6),subset_two(:,5:6));
         
         ALL_Indexed = [];
         for YT = 1:length(Index)
             row = Index(YT);
             distance = Distance(YT);
             FA_sub = Search_Data(Search_Data(:,3) == ARR(R-1),:);
             Previous_ID = FA_sub(row,9);
             Indexed = horzcat(Previous_ID, distance);
             ALL_Indexed = vertcat(ALL_Indexed, Indexed);   
         end
         
         NumRep_IDs = length(ALL_Indexed(:,1)) - length(unique(ALL_Indexed(:,1)));
         for Rep_IDs = 1:NumRep_IDs
            if length(ALL_Indexed(:,1)) == length(unique(ALL_Indexed(:,1)))
            else
                Values = unique(ALL_Indexed(:,1));
                
                if length(Values) == 1
                repeatedvalues = Values;
                else
                countofunique = hist(ALL_Indexed(:,1), Values);
                ...
                for new_ID_check = 0
                   ...
                   for repeat_length = 1:run
                       ID_check = ALL_Indexed(ALL_Indexed(:,1) == (repeatedvalues(repeat_length)),:);
                       max_distance = max(ID_check(:,2));
                       furthest_away = ID_check(ID_check(:,2) == max_distance,:);
                       new_ID = max(New_sub(:,9)) + Rep_IDs;
                       [row, col] = find(ALL_Indexed(:,2) == max_distance);
                       ALL_Indexed(row,1) = new_ID;
                   end
                end
            end
        ...
        Index = ALL_Indexed;
         ...
         New_subA = horzcat(subset_two, Index(:,1), Distance);
