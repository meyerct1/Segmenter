%function [] = MultiChannelSegmentation_v12()
...
%Number of levels for cytoplasmic segmentation using otsu's method
%If too high, the code will walk down until a convergent level is found.
numLevels = 20;  
%Image extension
imExt = '.tiff';
...
nuclear_segment = 1;
nuclear_segment_factor = 5;


%Clear the cells/nuclei touching border?
cl_border = 1;

%factor for smooth the cytoplasmic segmentation
smoothing_factor = -1;
...
%The noise filter disk used is 5 for 20X and 10 for 40X.
noise_disk = 7;
...
for i = 1:numCh
    ...
    if (cidrecorrect)
       Cyto.(chnm).CIDREmodel.v = csvread(['/home/xnmeyer/Documents/Lab/TNBC_Project/Experiments/Models_CIDRE_BaysSeg/20X_GREEN_2048/cidre_model_v.csv']);
       Cyto.(chnm).CIDREmodel.z = csvread(['/home/xnmeyer/Documents/Lab/TNBC_Project/Experiments/Models_CIDRE_BaysSeg/20X_GREEN_2048/cidre_model_z.csv']);
    end
end
...
info = imfinfo(char(NUC.filnms(1)));
...
%{
    im_name = cell(1,1)
    im_name{1} = ['Nuc/20151214151353-076-R02-C03' imExt];
    im_name{2} = ['Nuc/20151214151353-078-R02-C03' imExt];
    im_name{3} = ['Nuc/20151214151353-080-R02-C03' imExt];
    im_name{4} = ['Nuc/20151214151353-082-R02-C03' imExt];
    for i = 4:4
        temp = strfind({NUC.filnms{:}},im_name{i});
...
%         end
    ...
    SegIm_array(SegIm_array(:,:,1) == 3) = 1; %slightly out of focus Nuclei
    ...
    if nuclear_segment==1
        Label_array = im2double(imdilate(SegIm_array(:,:,1),strel('disk',nuclear_segment_factor)));
    else
        ...
        for q = 1:numCh
            ...
            try num = multithresh(Im_array(:,:,q+1),numLevels);
            catch exception
                while cnt<numLevels
                  try 
    ...
    p	= regionprops(CytoLabel,'PixelIdxList','Perimeter','Eccentricity');
    ...
    for q = 1:numCh
        ...
        if size(p,1) ~= 0
            for k = 1:size(p,1)
                ...
                Eccentricity(m) = p(k).Eccentricity;
                m= m+1;
            end
        end
        ...
        CO.(chnm).Eccentricity = Eccentricity;
        CO.(chnm).cellId = 1:size(p,1);
    end
    %Store similar information for the Nucleus segmentation
    p	= regionprops(Nuc_label,'PixelIdxList','Perimeter','Eccentricity');
    Int = []; Area = []; Perimeter = []; Eccentricity = [];
    m = 1;
    tempIm = Im_array(:,:,1);
    if size(p,1) ~= 0
        for k = 1:size(p,1)
    ...
    CO.Nuc.Eccentricity = Eccentricity;
