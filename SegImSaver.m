...
numCh = 1; %Number of fluourescent channels not including nuclear channel
imExt = '.jpg';
...
%future code?  Currently don't use...
corCH = 'CH_3';
corVal = 29;
...
%          end
    ...
    subplot(2,2,1)
    imshow(Im_array(:,:,1),[])
    str = sprintf('mCherry Nucleus image:%i\n%s',idx(i),char(NUC.filnms(idx(i))));
    title(str)
    subplot(2,2,2)
    imshow(Im_array(:,:,2),[])
    title('CD44')
    ...
    imshow(Im_array(:,:,4),[])
    title('CD24')
    str = sprintf('Segmentation_Images/Initial Channels im_%i',idx(i));
    savefig(h,str)
    close(h)
...
%         end
    ...
    title('CD44')
    subplot(2,2,2)
    imshow(SegIm_array(:,:,3),[]);
    title('EpCAM')
    subplot(2,2,3)
    imshow(SegIm_array(:,:,4),[]);
    title('CD24')
    subplot(2,2,4)
    imshow(Label_array,[]);
    title('Merged Channel')
    ...
    str = sprintf('Segmentation_Images/Cytoplasm segmentation im_%i',idx(i));
    savefig(h,str)
    close(h)
